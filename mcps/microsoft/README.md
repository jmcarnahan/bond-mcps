...
`search_people` needs `User.ReadBasic.All`, which the consented default scope set does not include, and `ensure_chat` needs `Chat.ReadWrite`. `get_profile` shares that requirement the moment it is given a `user` or asked for a `photo`; without the scope it returns `directory_scope_missing` and nothing else. Both are admin-gated in an organisational tenant. The order below matters: Entra evaluates a consent request as one bundle, so widening `MS_SCOPES` before the admin has granted the scope walls every sign-in behind "Approval required", mail included (the PR #18 release note explains the rule).
