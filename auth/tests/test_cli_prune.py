...
class TestPruneOAuth:
    def test_deletes_expired_pending_auth(self, fresh_db):
        ...
        with get_session() as s:
            s.add(OAuthPendingAuth(
                bond_state="expired-pending",
                client_id="cid", redirect_uri="x",
                code_challenge="c", upstream_code_verifier_encrypted=b"x",
                key_version=1, expires_at=now - timedelta(hours=1),
            ))
            s.add(OAuthPendingAuth(
    ...
    def test_deletes_expired_auth_codes(self, fresh_db):
        ...
        with get_session() as s:
            s.add(OAuthAuthCode(
                code_hash="expired", client_id="cid", user_key="u",
                code_challenge="c", redirect_uri="x",
                expires_at=now - timedelta(hours=1),
            ))
            s.add(OAuthAuthCode(
                code_hash="fresh", client_id="cid", user_key="u",
                code_challenge="c", redirect_uri="x",
    ...
    def test_deletes_long_revoked_refresh_tokens(self, fresh_db):
        ...
        with get_session() as s:
            ...
            s.add(OAuthRefreshToken(
                token_hash="old-revoked", client_id="cid", user_key="u",
                revoked_at=now - timedelta(days=14),
                expires_at=now + timedelta(days=16),
            ))
            # Revoked 1 day ago — within grace
            s.add(OAuthRefreshToken(
                token_hash="recent-revoked", client_id="cid", user_key="u",
                revoked_at=now - timedelta(days=1),
                expires_at=now + timedelta(days=29),
            ))
            # Active
            s.add(OAuthRefreshToken(
                token_hash="active", client_id="cid", user_key="u",
    ...
    def test_deletes_idle_dcr_clients_but_keeps_active(self, fresh_db):
        ...
        with get_session() as s:
            ...
            s.add(OAuthClient(
                client_id="idle", redirect_uris=["http://localhost/cb"],
                grant_types=["authorization_code"], response_types=["code"],
                is_static=False, created_at=now - timedelta(days=60),
            ))
            # Active client — also old, but has a recent refresh token
            s.add(OAuthClient(
                client_id="active", redirect_uris=["http://localhost/cb"],
                grant_types=["authorization_code"], response_types=["code"],
                is_static=False, created_at=now - timedelta(days=60),
            ))
            s.add(OAuthRefreshToken(
                token_hash="rt-for-active", client_id="active", user_key="u",
                created_at=now - timedelta(days=2),
                expires_at=now + timedelta(days=28),
            ))
            # Static client — never pruned regardless of age
            s.add(OAuthClient(
                client_id="static", redirect_uris=["http://localhost/cb"],
                grant_types=["authorization_code"], response_types=["code"],
                is_static=True, created_at=now - timedelta(days=180),
            ))
...
class TestRevokeTokens:
    def test_revokes_only_for_specified_user(self, fresh_db):
        ...
        with get_session() as s:
            s.add(OAuthRefreshToken(
                token_hash="alice-1", client_id="cid", user_key="alice",
                expires_at=now + timedelta(days=30),
            ))
            s.add(OAuthRefreshToken(
                token_hash="alice-2", client_id="cid", user_key="alice",
                expires_at=now + timedelta(days=30),
            ))
            s.add(OAuthRefreshToken(
                token_hash="bob-1", client_id="cid", user_key="bob",
                expires_at=now + timedelta(days=30),
            ))
        ...
        with get_session() as s:
            revoked = s.query(OAuthRefreshToken).filter(
                OAuthRefreshToken.revoked_at.isnot(None)
            ).count()
            assert revoked == 2  # both alice rows
            bob_revoked = s.query(OAuthRefreshToken).filter(
    ...
    def test_idempotent_on_already_revoked(self, fresh_db):
        ...
        with get_session() as s:
            s.add(OAuthRefreshToken(
                token_hash="already-rev", client_id="cid", user_key="alice",
